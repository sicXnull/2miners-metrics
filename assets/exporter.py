...
class PromExporter:
	...
	def fetchData(self):
		...
		for key in self.endpoints.keys():
			...
			if key == "price":
				...
				logger.info(f"url : https://{self.endpoints[key]}")
				...
				self.data[key]['mining_profitability_percent'] = round((self.data[key]['mining_profitability'] / self.data['2miners'][f'unpaid_last_24_hr_{self.currency}']) * 100,2)
	...
	@property
	def metrics(self):
		logger.info(f"Formatting Metrics Data")
		lines = ""
		for key in self.data.keys():
			lines = f"{lines}{key} {self.data[key]}\n"
		return lines
